...
class ExperienceGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Experience entity")
        ...
        .repeat(2) {
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "experienceNom":"SAMPLE_TEXT", "experienceTypeExperience":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_experience_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
